...
- From the Cloud VM's network region, some data sources are **geo-blocked** (HTTP `403` / "Amazon CloudFront ... configured to block access from your country"). This mainly affects exchange-backed pages like `/monitor` and `/screener`, which can render empty. It is a network-location limitation, not a code bug; sections backed by other sources (Intelligence, Seasonality, Backtest via yfinance, News) work fine.
